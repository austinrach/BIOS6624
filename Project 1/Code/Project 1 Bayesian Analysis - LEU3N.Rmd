...
Checking for clinically meaningful difference of LEU3N for hard drug users (50)
```{r}

draws_df_LEU3N <- as_draws_df(fit.LEU3N$draws())

# double checking find column index of hard drugs
k <- which(colnames(X.LEU3N) == "hard_drugs1")

beta_name <- paste0("beta[", k, "]")

# posterior probability of clinically meaningful difference in CD4 count between drug users
prop.table(table(draws_df_LEU3N$'beta[3]' > 50 | draws_df_LEU3N$'beta[3]' < -50))

pclin_LEU3N <- mean(abs(draws_df_LEU3N[[beta_name]]) > 50)
```

The posterior probability that the magnitude of the difference exceeds 50 cells was 99.97%, indicating very strong evidence of a clinically meaningful reduction in CD4 count among hard drug users.
...
Checking for clinically meaningful difference of LEU3N for hard drug users (50) adjusting for adherence
```{r}

draws_df_LEU3N_ADH <- as_draws_df(fit_LEU3N_ADH$draws())

# double checking find column index of hard drugs
k <- which(colnames(X.LEU3N.ADH) == "hard_drugs1")

beta_name <- paste0("beta[", k, "]")

# posterior probability of clinically meaningful difference in CD4 count between drug users
prop.table(table(draws_df_LEU3N_ADH$'beta[3]' > 50 | draws_df_LEU3N_ADH$'beta[3]' < -50))

pclin_LEU3N_ADH <- mean(abs(draws_df_LEU3N_ADH[[beta_name]]) > 50)
```

The posterior probability that the magnitude of the difference exceeds 50 cells was 99.99%, indicating extremely strong evidence that hard drug use is associated with a clinically meaningful decrease in CD4 count, controlling for adherence.
...
```{r}
saveRDS(summary_table_LEU3N, file = "/Users/austinra/BIOS6624/Project 1/Data Processed/summary_table_leu3n.rds")
saveRDS(summary_table_LEU3N_ADH, file = "/Users/austinra/BIOS6624/Project 1/Data Processed/summary_table_leu3n_adh.rds")
...
saveRDS(
  ...
  file = file.path("/Users/austinra/BIOS6624/Project 1/Data Processed/bayes_results_leu3n_primary.rds")
)
...
saveRDS(
  list(
    ...
    model = "adh",
    beta_name = beta_name,
    p_clin = pclin_LEU3N_ADH,
    loo = loo_res_LEU3N_ADH
  ),
  file = file.path("/Users/austinra/BIOS6624/Project 1/Data Processed/bayes_results_leu3n_adh.rds")
)
```
