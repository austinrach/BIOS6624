...
Checking for clinically meaningful difference in log VLOAD between hard drug users
```{r}
draws_df_VLOAD <- as_draws_df(fit.VLOAD$draws())

# double checking find column index of hard drugs
k <- which(colnames(X.VLOAD) == "hard_drugs1")

beta_name <- paste0("beta[", k, "]")

# posterior probability of clinically meaningful difference in log vload between drug users
prop.table(table(draws_df_VLOAD$'beta[3]' > 0.5 | draws_df_VLOAD$'beta[3]' < -0.5))

pclin_VLOAD <- mean(abs(draws_df_VLOAD[[beta_name]]) > 0.5)

```
There is a 27.9% posterior probability that hard drug use is associated with at least a 0.5 log-unit difference in viral load. There is little evidence of a meaningful effect.
...
```{r}
draws_VLOAD <- as_draws_array(fit.VLOAD$draws())  # dimensions: iterations x chains x parameters
draws_df_VLOAD <- as_draws_df(fit.VLOAD$draws())  # tidy format for bayesplot / ggplot
...
Checking for clinically meaningful difference in log VLOAD between hard drug users adjusted for adherence
```{r}
draws_df_VLOAD_ADH <- as_draws_df(fit_VLOAD_ADH$draws())

# double checking find column index of hard drugs
k <- which(colnames(X.VLOAD.ADH) == "hard_drugs1")

beta_name <- paste0("beta[", k, "]")

# posterior probability of clinically meaningful difference in log vload between drug users
prop.table(table(draws_df_VLOAD_ADH$'beta[3]' > 0.5 | draws_df_VLOAD_ADH$'beta[3]' < -0.5))

pclin_VLOAD_ADH <- mean(abs(draws_df_VLOAD_ADH[['beta[3]']]) > 0.5)

```
There is 1.4% posterior probability that hard drug use is associated with at least a 0.5 log-unit difference in viral load. There is little evidence of a meaningful effect after adjusting for adherence.


Save estimates for summary tables
```{r}
saveRDS(summary_table_VLOAD, file = "/Users/austinra/BIOS6624/Project 1/Data Processed/summary_table_vload.rds")
saveRDS(summary_table_VLOAD_ADH, file = "/Users/austinra/BIOS6624/Project 1/Data Processed/summary_table_vload_adh.rds")
```

Save clinically meaningful posterior prob and loo-cv
```{r}
saveRDS(
  list(
    outcome = "VLOAD",
  ...
  file = file.path("/Users/austinra/BIOS6624/Project 1/Data Processed/bayes_results_vload_primary.rds")
)
...
saveRDS(
  list(
    ...
    model = "adh",
    beta_name = beta_name,
    p_clin = pclin_VLOAD_ADH,
    loo = loo_res_VLOAD_ADH
  ),
  file = file.path("/Users/austinra/BIOS6624/Project 1/Data Processed/bayes_results_vload_adh.rds")
)
