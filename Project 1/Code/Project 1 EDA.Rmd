...
df_base <- df_all4 %>%
  filter(years == 0) %>%
  select(newid, hard_drugs, ADH, age, BMI, SMOKE, education, race,
         VLOAD, LEU3N, AGG_PHYS, AGG_MENT, log_vload, sqrt_leu3n)
...
table1(
  ~ VLOAD_0 + VLOAD_2 + LEU3N_0 + LEU3N_2 + AGG_PHYS_0 + AGG_PHYS_2 + AGG_MENT_0 + AGG_MENT_2
    + ADH + age + BMI + SMOKE + education + race
  | hard_drugs,
  data = df_analysis
)
