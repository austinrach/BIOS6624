...
```{r,  include=FALSE}
df <- read.csv("/Users/austinra/BIOS6624/Project 1/Data Raw/hiv_6624_final.csv")
...
#Collapse SMOKE to to current smoker and not current smoker
df_all4 <- df_all4 %>% 
...
#Create logvload variable
df_all4$log_vload <- log10(df_all4$VLOAD)
```
...
#Filter out missing bmi
df_analysis <- filter(df_analysis, !is.na(bmi))

# Write dataset to folder for analysis
openxlsx::write.xlsx(df_analysis, "/Users/austinra/BIOS6624/Project 1/Data Processed/Analysis Dataset.xlsx")
...
df_analysis <- df_analysis %>% 
  mutate(hard_drugs =
           factor(hard_drugs,
                  levels = c(0,1),
                  labels = c("Not a Hard Drug User", "Hard Drug User")),
         education = 
           factor(education, 
             levels = c(0, 1), 
...
label(df_analysis$log_vload_0)    <- "Log Viral Load at Baseline"
label(df_analysis$log_vload_2)    <- "Log Viral Load at Year 2"
...
tbl <- df_analysis %>%
  select(
    log_vload_0, log_vload_2, LEU3N_0, LEU3N_2,
  ...
  bold_labels() %>%
  modify_caption("**Baseline characteristics by hard drug use**")
tbl_latex <- tbl %>%
  ...
  kableExtra::kable_styling(
    ...
    general = "Viral load values are presented on the log10 scale.",
    threeparttable = TRUE
  )
```
