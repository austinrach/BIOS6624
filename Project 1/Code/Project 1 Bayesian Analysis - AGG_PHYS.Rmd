...
Checking for clinically meaningful difference (plus/minus 2) of AGG_PHYS score for hard drug users 
```{r}

draws_df_AGG_PHYS <- as_draws_df(fit.AGG_PHYS$draws())

# double checking find column index of hard drugs
k <- which(colnames(X.AGG_PHYS) == "hard_drugs1")

beta_name <- paste0("beta[", k, "]")

# posterior probability of clinically meaningful difference in physical QOL score between drug users
prop.table(table(draws_df_AGG_PHYS$'beta[3]' > 2 | draws_df_AGG_PHYS$'beta[3]' < -2))

pclin_AGG_PHYS <- mean(abs(draws_df_AGG_PHYS[[beta_name]]) > 2)
```
The posterior probability that the magnitude of the difference exceeds 2 points was 83.6%, indicating strong evidence for a clinically meaningful reduction in AGG_PHYS score among hard drug users. There remains approximately  16.4% posterior probability that the true difference is smaller than the clinically meaningful threshold.
...
Checking for clinically meaningful difference of physical QOL score- 2


```{r}

draws_df_AGG_PHYS_ADH <- as_draws_df(fit_AGG_PHYS_ADH$draws())

# double checking find column index of hard drugs
k <- which(colnames(X.AGG_PHYS.ADH) == "hard_drugs1")

beta_name <- paste0("beta[", k, "]")

# posterior probability of clinically meaningful difference in physical QOL score between drug users
prop.table(table(draws_df_AGG_PHYS_ADH$'beta[3]' > 2 | draws_df_AGG_PHYS_ADH$'beta[3]' < -2))

pclin_AGG_PHYS_ADH <- mean(abs(draws_df_AGG_PHYS_ADH[[beta_name]]) > 2)
```
The posterior probability that the magnitude of the difference exceeds 2 points was 86.9%, indicating strong evidence for a clinically meaningful reduction in AGG_PHYS score among hard drug users controlling for adherence. There remains approximately 13.1% posterior probability that the true difference is smaller than the clinically meaningful threshold.
...
```{r}
saveRDS(summary_table_AGG_PHYS, file = "/Users/austinra/BIOS6624/Project 1/Data Processed/summary_table_agg_phys.rds")
saveRDS(summary_table_AGG_PHYS_ADH, file = "/Users/austinra/BIOS6624/Project 1/Data Processed/summary_table_agg_phys_adh.rds")

saveRDS(list(
    outcome = "AGG_PHYS",
    model = "primary",
    beta_name = beta_name,
    p_clin = pclin_AGG_PHYS,
    loo = loo_res_AGG_PHYS
  ),
  file = file.path("/Users/austinra/BIOS6624/Project 1/Data Processed/bayes_results_agg_phys_primary.rds")
)
...
saveRDS(
  list(
    ...
    model = "adh",
    beta_name = beta_name,
    p_clin = pclin_AGG_PHYS_ADH,
    loo = loo_res_AGG_PHYS_ADH
  ),
  file = file.path("/Users/austinra/BIOS6624/Project 1/Data Processed/bayes_results_agg_phys_adh.rds")
)
```
