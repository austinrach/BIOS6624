...
df_mod$booklet_minutes <- lubridate::hour(df_mod$Booket..Clock.Time) * 60 +
                          lubridate::minute(df_mod$Booket..Clock.Time)

df_mod$cap_minutes <- lubridate::hour(df_mod$MEMs..Clock.Time) * 60 +
                      lubridate::minute(df_mod$MEMs..Clock.Time)

df_mod$wake_minutes <- lubridate::hour(df_mod$Sleep.Diary.reported.wake.time) * 60 +
...
df_mod$cap_interval_minutes <-
  df_mod$cap_minutes - df_mod$wake_minutes

df_mod$booklet_interval_minutes <-
  df_mod$booklet_minutes - df_mod$wake_minutes
...
df_q1 <- filter(df_mod, !is.na(cap_interval_minutes) & !is.na(booklet_interval_minutes))
```


```{r}
#Scatter plot of cap time (minutes) from waking up and booklet time (minutes) since waking up (continuous)
ggplot(df_q1, aes(x = cap_interval_minutes, y = booklet_interval_minutes)) +
...
ggplot(df_q1, aes(x = cap_interval_minutes, y = booklet_interval_minutes)) +
  ...
  theme_minimal() +
  coord_equal()+ 
  coord_cartesian(xlim = c(min(df_q1$cap_interval_minutes),50),
                  ylim = c(min(df_q1$booklet_interval_minutes), 50))
```


Find the difference between recorded sampling time and recorded cap time, plot this distribution
```{r}
df_q1$booklet_cap_difference <- df_q1$booklet_interval_minutes - df_q1$cap_interval_minutes
mean(df_q1$booklet_cap_difference)
ggplot(df_q1,
...
```{r}
q1.lmm <- lmer(booklet_interval_minutes ~ cap_interval_minutes + (1|SubjectID), 
               data = df_q1)
summary(q1.lmm)
confint(q1.lmm)
```
...
dftest <- df_mod %>%
  ...
  mutate(
    collection_time = case_when(
      as.character(Collection.Sample) == '2' ~ 30,
      as.character(Collection.Sample) == '4' ~ 60*10,
      TRUE ~ NA
    ),
...
```{r}
#Filter to just Collection.Sample == 2 and Collection.Sample == 4
df_q2 <- df_mod %>%
  filter(as.character(Collection.Sample) %in% c("2", "4"))

table(df_q2$Collection.Sample, df_q2$booklet_interval_minutes, useNA = "ifany")
# 6 MISSING BOOKLET for 30 mins, 13 for 600 mins
#Create collection time var to grab samples we're interested in and booklet and cap deviation times from those 
table(df_q2$Collection.Sample, df_q2$cap_interval_minutes, useNA = "ifany")
#23 missing cao for 30, 10 fir 600
df_q2_book <- filter(df_q2, !is.na(Booket..Clock.Time))
df_q2_cap <- filter(df_q2, !is.na(MEMs..Clock.Time))

df_q2_cap$cap_dev <- ifelse(df_q2_cap$Collection.Sample == "2", df_q2_cap$cap_interval_minutes - 30, df_q2_cap$cap_interval_minutes - 600)

df_q2_book$book_dev <- ifelse(df_q2_book$Collection.Sample == "2", df_q2_book$booklet_interval_minutes - 30, df_q2_book$booklet_interval_minutes - 600)

df_q2_cap <- df_q2_cap %>%
  mutate(
    cap_good = if_else(
      as.character(Collection.Sample) == "2",
      cap_interval_minutes >= 22.5 & cap_interval_minutes <= 37.5,
      cap_interval_minutes >= 592.5 & cap_interval_minutes <= 607.5
    )
  )
table(df_q2_cap$cap_good)
```
...
#Remove lab values
df_q3 <- filter(df_q3,DHEA..nmol.L. < 5.205)
```
