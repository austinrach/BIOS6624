...
The investigator stated that levels of cortisol are expected to increase sharply from waking to 30 minutes after waking and then decline more slowly over the rest of the day.
...
kable(cortisol_table, digits = 3,
      caption = "Linear Mixed Model of Log Cortisol on Minutes Since Waking - Raw Estimates")
kable(cortisol_table_exp, digits = 3,
      caption = "Linear Mixed Model of Log Cortisol on Minutes Since Waking - Transformed Estimates")
```
...
q3_dhea.lmm.knot <- lmer(
  log_dhea ~ wake_30 + after_30  + (1 | SubjectID),
  data = df_q3
)

summary(q3_dhea.lmm.knot)
```
