...
df_mod$Collection.Date <- as.Date(df_mod$Collection.Date, format = "%m/%d/%Y")
#Collection.Sample should be Categorical
df_mod$Collection.Sample <- as.factor(df_mod$Collection.Sample)
...
df_mod$cap_interval_mins <- as.numeric(df_mod$MEMs..Clock.Time - df_mod$Sleep.Diary.reported.wake.time, units = "mins")
df_mod$booklet_interval_mins <- as.numeric(df_mod$Booket..Clock.Time - df_mod$Sleep.Diary.reported.wake.time, units = "mins")
```


Check each SubjectID has proper number of samples
```{r}
summary <- df_mod %>% group_by(SubjectID, DAYNUMB) %>% summarize(n = n()) %>% ungroup()
#Subject 3029 has 12 observations on day 3 - let's see what's going on
problem_subj <- filter(df_mod, SubjectID == 3029)
...
Save processed data
```{r}
#write.xlsx(df_mod, "/Users/austinra/Documents/MS BIOS/Spring 2026/BIOS 6624/Project 0/Data Processed/Project0_ProcessedData.xlsx")
```
...
df_q2 <- filter(df_mod, Collection.Sample == 2 | Collection.Sample == 4)
summary(df_q2)
#33 missing caplet time since wake, 19 missing booklet time since wake
