...
confint(q1.lmm)
```
For every 1 minute increase in recorded cap time, recorded booklet time increases by 0.9956 minutes (95% CI: 0.982, 1.010, p-value: <0.0001). This suggests there is strong agreement between recorded cap time and recorded booklet time. The intercept suggests there is bias though, subjects recorded sample collection in their booklet about 6.816 (95% CI: -12.527, -1.150, p-value: 0.0215) minutes earlier than recorded sample collection by the electronic cap. 
...
df <- df %>%
  mutate(
    booklet_good = case_when(
      Collection.Sample == 2 & between(booklet_interval_mins, 22.5, 37.5) ~ TRUE,
      Collection.Sample == 2 ~ FALSE,
      Collection.Sample == 4 & between(booklet_interval_mins, 592.5, 607.5) ~ TRUE,
      Collection.Sample == 4 ~ FALSE,
      TRUE ~ NA
    ),
    booklet_adequate = case_when(
      Collection.Sample == 2 & between(booklet_interval_mins, 15,45) ~ TRUE,
      Collection.Sample == 2 ~ FALSE,
      Collection.Sample == 4 & between(booklet_interval_mins, 545, 615) ~ TRUE,
      Collection.Sample == 4 ~ FALSE,
      TRUE ~ NA
    ),
    cap_good = case_when(
      Collection.Sample == 2 & between(cap_interval_mins, 22.5, 37.5) ~ TRUE,
      Collection.Sample == 2 ~ FALSE,
      Collection.Sample == 4 & between(cap_interval_mins, 592.5, 607.5) ~ TRUE,
      Collection.Sample == 4 ~ FALSE,
      TRUE ~ NA
    ),
    cap_adequate = case_when(
      Collection.Sample == 2 & between(cap_interval_mins, 15, 45) ~ TRUE,
      Collection.Sample == 2 ~ FALSE,
      Collection.Sample == 4 & between(cap_interval_mins, 545, 615) ~ TRUE,
      Collection.Sample == 4 ~ FALSE,
      TRUE ~ NA
    )
  ) %>%
...
#DHEA 
x <- filter(df,DHEA..nmol.L. == 5.205)
#SubjectID 3037 needs to be be removed as it is at the upper limit multiple times, suggesting something is wrong
df <- filter(df, SubjectID != 3037)
```


```{r}
#Visualize cortisol
hist(df$Cortisol..nmol.L., main = "Histogram of Cortisol (nmol/L")
#Heavy right skew - let's look at a log transformation
hist(log(df$Cortisol..nmol.L.), main = "Histogram of Cortisol (nmol/L")
```
```{r}
#Visualizing DHEA
hist(df$DHEA..nmol.L.)
...
Significant outlier
```{r}
ggplot(df, aes(x = cap_interval_mins, y = Cortisol..nmol.L., group = SubjectID)) +
  geom_line() +
  theme_minimal(base_size = 14) +
  labs(
...
```{r}
ggplot(df, aes(x = cap_interval_mins, y = df$DHEA..nmol.L., group = SubjectID)) +
  geom_line() +
  theme_minimal(base_size = 14) +
  labs(
    x = "Minutes since waking (cap)",
...
(exp(fixef(q3_cortisol.lmm)) - 1) * 100
```
